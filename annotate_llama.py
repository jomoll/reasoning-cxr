...
prompt_base = (
    ...
    "2. Do not include any findings that are not explicitly stated in the clinical data. Do not omit any findings.\n"
    "3. Each step should flow logically: describe where, then what, then so what.\n"
    "4. Where applicable, apply the following severity scales.\n"
    "   4a. For pulmonary edema, apply: (mild) vascular redistribution only; (moderate) interstitial edema (Kerley B lines, peribronchial cuffing, septal thickening); (severe) alveolar edema (bat-wing consolidations, alveolar fluid, air bronchograms).\n"
    "   4b. For pleural effusion, apply: (mild) blunting of costophrenic angle only; (moderate) small layering effusion (<1/3 hemithorax height); (severe) moderate-large effusion (>1/3 with meniscus and possible mediastinal shift).\n"
    "   4c. For pneumothorax, apply: (mild) small (<2 cm apical rim, no mediastinal shift); (moderate) moderate (2-4 cm rim, no shift); (severe) large/tension (>4 cm rim or any mediastinal shift).\n"
    "   4d. For cardiomegaly, use cardiothoracic ratio: normal if not enlarged, mild enlargement or marked enlargement.\n"
    "   4e. For consolidation patterns, distinguish: (mild) focal/lobar; (moderate) multifocal; (severe) diffuse/alveolar (e.g. ARDS pattern).\n"
    "5. Keep Actions and Observations bulleted for readability.\n\n"
    "6. Do not simply repeat phrases from the example; it's for reference only.\n\n"
